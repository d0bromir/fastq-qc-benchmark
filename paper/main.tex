...
its CPU counterpart on this workload profile---GPU overhead is variable
(ranging from~$-0.5$ to~$+5$\,s depending on thread count) and provides
no consistent benefit when the pipeline bottleneck is gzip decompression
and sequential I/O.  We discuss
...
cards ($\sim$4\% SM, $\sim$428\,MiB each).  The repository (d0bromir/fastp, GitHub) targets Compute
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    fastp d0bromir (GPU, 8T)    &  7.4\,s &  103\,s &  151\,s &  166\,s \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrrrr}
    ...
    fastp d0bromir (GPU) & 616.7 & 327.3 & 170.4 & 102.7 & 104.0 & 105.8 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Wall-clock time vs.\ worker thread count for fastp variants on
    ...
    reference.  Both CPU variants saturate at 8T; the GPU variant converges
    to near-parity with the CPU binary at 8--16T, with $+5$\,s overhead at 4T.}
  \label{fig:scaling}
\end{figure}
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    fastp d0bromir (GPU, 8T) & 4.23$\times$ & 4.21$\times$ & 4.24$\times$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
The GPU-enabled fastp fork shows variable overhead relative to the
CPU-forced baseline (Table~\ref{tab:gpuoverhead}): $+5.0$\,s at 4\,threads,
$-0.5$\,s at 8\,threads (GPU marginally faster), and $+0.5$\,s at
16\,threads.  The elevated overhead at 4T reflects GPU kernel launch latency:
an idle CUDA kernel dispatch on an A100 incurs ${\approx}5$--$20\,\mu$s of
host-side overhead per launch, and the fastp GPU fork dispatches read-batch
kernels across both A100 cards via a round-robin selector.  At 4T fewer
batches are available per unit time, so per-launch latency dominates.
At 8T and above, the benefit and cost of GPU dispatch roughly cancel,
yielding near-zero net overhead ($\le 1$\,s).
...
\begin{itemize}
  ...
  \item \textbf{fastp GPU mode} shows even stronger non-linearity: on the panel
    file the GPU binary is \emph{twice as slow} as the CPU binary (7.4\,s vs.
    ...
    WGS files this fixed overhead becomes negligible, and the steady-state
    per-batch dispatch cost is near-zero ($\le 1$\,s) at the optimal 8T
    configuration.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{GPU acceleration does not benefit FASTQ QC} at this data scale
    with the current d0bromir fastp v1.2.2 implementation.  GPU overhead is
    variable ($-0.5$ to $+5$\,s vs.\ CPU-forced) and provides no consistent
    benefit because the workload is I/O-bound and arithmetic intensity is too
    low to amortise PCIe transfer costs.
