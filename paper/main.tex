...
compiled with \texttt{-march=native} to exploit ARM Advanced SIMD (NEON)
vector instructions on the Neoverse N1.
(Note: SVE is not exposed by the hypervisor on this platform;
the kernel \texttt{Features} field lists \texttt{asimd} but not \texttt{sve}.)
...
To confirm that the GPU modifications do not alter filtering logic or quality
statistics, we ran both the CPU-only binary (\texttt{fastp\_d0bromir\_cpu}) and
the GPU binary (\texttt{fastp\_d0bromir\_gpu}) on all four test files
($-$w~8, output suppressed with \texttt{/dev/null}) and compared their
\texttt{.json} reports field-by-field.  Eighteen statistics were checked per
dataset: total reads, total bases, Q20/Q30 bases and rates, and GC content
(before and after filtering), plus the five filtering-result counters (passed,
low-quality, too many N, too short, too long).  Integer fields were required to
match exactly; floating-point rates were required to agree within a relative
tolerance of $0.01\%$.  For the Panel dataset we additionally compared the
compressed FASTQ output byte-for-byte via MD5 checksum.

All 18 fields matched exactly for every dataset, and the Panel FASTQ MD5 was
identical between both binaries
(\texttt{26fb6cd0258306885015c3880872f231}).  Full per-field comparison
logs are available in the repository at
\texttt{results/validation/validation\_report.txt} (commit~\texttt{d6b84ac}).
...
\texttt{-march=native} flag exposes on the Neoverse N1 Advanced SIMD (NEON) units.
...
libisal and libdeflate (both compiled with NEON/Advanced SIMD intrinsics via
...
We verified that the GPU modifications produce numerically identical output to
the unmodified CPU binary (Section~\ref{sec:correctness}).  All 18 quality
statistics fields (exact integer counts and floating-point rates) matched on
every test dataset; the Panel FASTQ output was bit-for-bit identical
(MD5~\texttt{26fb6cd0\ldots}).
...
(NEON) intrinsics; the libisal decompression library similarly targets NEON
on this platform (the Neoverse N1's SVE capability is not exposed by the
hypervisor; SVE2 requires ARMv9 / Neoverse N2 or later).  Observed throughput on WGS-6G at 8T (${\approx}60$\,MB/s
...
Re-evaluating on a next-generation Neoverse N2 (ARMv9) platform with SVE2
SIMD support may push the fastp saturation point to higher thread counts;
the current Neoverse N1 has SVE2-capable silicon but it is not exposed by
the hypervisor on this system.  SVE2 vectorisation of the inner quality-score
accumulation loop is not yet implemented upstream.  The GPU-accelerated BGZF decompressor implemented in this fork
